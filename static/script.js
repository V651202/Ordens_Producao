...
async function carregarOrdens() {
        ...
        corpo.innerHTML = ordens.map(ordem => `
            <tr id="linha-${ordem.id}">
                ...
                <td>
                    ...
                    <button class="btn-editar" onclick="editarOrdem(${ordem.id})">
                        Editar
                    </button>
    ...
    setTimeout(() => {
        document.querySelectorAll("#corpo-tabela tr").forEach((linha, i) => {
...
async function editarOrdem(id) {
    const ordem = await fetch(`${API_URL}/ordens/${id}`);
    const dados = await ordem.json();

    // Abre prompts simples (rápido e funcional)
    const novoProduto = prompt("Editar produto:", dados.produto);
    const novaQtd = prompt("Editar quantidade:", dados.quantidade);
    ...
    try {
        ...
        if (resposta.ok) {
            exibirMensagem(`Ordem #${id} atualizada!`, 'sucesso');
            await carregarOrdens();
        } else {
            exibirMensagem(resp.erro || 'Erro ao editar.', 'erro');
        }
