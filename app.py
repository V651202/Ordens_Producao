...
def criar_ordem():
    ...
    #Buscar o registros que foi recém-criado
    conn = get_connection()
    cursor = conn.cursor()
...
def remover_ordem(ordem_id):
    ...
    Parâmetros de URL:
        ordem_id (int) : ID a ordem a ser removida.
    ...
    return jsonify({'mensagem' : f'Ordem de numero {ordem_id} ({nome_produto_apagado}) removida com sucesso!', 'id_removido' :
                    ordem_id}), 200
